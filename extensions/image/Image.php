...
class Image {
    ...
    public function createThumb($width,$height){
        ...
        if(file_exists($image) && is_file($image)){
	        ...
	        $dir = "resources/images/".$width."x".$height;
	        $thumbPath = Yii::getPathOfAlias('webroot')."/".$dir."/".$imgName;
	        if(!is_dir($thumbPath)){
	            $arrFolder = explode("/",$dir);
	            $pFolder = Yii::getPathOfAlias('webroot');
	            foreach($arrFolder as $d){
	                $pFolder .= "/".$d;
	                if(!is_dir($pFolder)){
	                    mkdir($pFolder);
	                }
	        ...
	        return Yii::app()->baseUrl."/".$dir."/".$imgName;
        }
